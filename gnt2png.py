...
def main(path, source_path, ratioA, ratioB):
    ...
    datafolder = os.path.join('datasets', filename + '-' + ratioA + '-' + ratioB)
    ...
    for x in np.arange(len(arr)):
        if len(os.listdir(trainA_path)) >= 1000:
            break
        ch = charlist[arr[x]]
        bitmap = bitmaplist[arr[x]]
        source_img = sourcelist[arr[x]]
        if arr[x] <= ntrainA and arr[x] <= ntrainB:
            imageio.imsave(os.path.join(trainA_path, str(ord(ch[0])) + '.png'), bitmap)
            imageio.imsave(os.path.join(trainB_path, str(ord(ch[0])) + '.png'), source_img)
        elif arr[x] > ntrainA and arr[x] <= ntrainB:
            imageio.imsave(os.path.join(testA_path, str(ord(ch[0])) + '.png'), bitmap)
            imageio.imsave(os.path.join(trainB_path, str(ord(ch[0])) + '.png'), source_img)
        elif arr[x] <= ntrainA and arr[x] > ntrainB:
            imageio.imsave(os.path.join(trainA_path, str(ord(ch[0])) + '.png'), bitmap)
            imageio.imsave(os.path.join(testB_path, str(ord(ch[0])) + '.png'), source_img)
        else:
            imageio.imsave(os.path.join(testA_path, str(ord(ch[0])) + '.png'), bitmap)
            imageio.imsave(os.path.join(testB_path, str(ord(ch[0])) + '.png'), source_img)
